...
public class SftpConnectionFactory implements PoolableObjectFactory
{
	...
	public static SftpClient createClient(ImmutableEndpoint endpoint) throws Exception
	{
		...
		try
		{
			...
			/* TODO: There is a problem if the SSHd uses a low value of "MaxStartups", which means
					 that if there is many new concurrent connections the server will respond with
					  "Connection reset", and thus we will get exceptions of type
					  "Session.connect: java.net.SocketException: Connection reset"...
					  Solutions: increase the MaxStartups on the server or fix a retry or use a exception-strategy(?)
			*/
