...
public class SftpConnector extends AbstractConnector
{
  ...
  private Map<EndpointURI, GenericObjectPool> pools = new HashMap<EndpointURI, GenericObjectPool>();
	...
	/** Value that can be set via the System property 'mule.sftp.transport.maxConnectionPoolSize'. If it's set the value is used instead of
	 * <i>maxConnectionPoolSize</i> */
	...
	static {
		...
		if(propValue != null) {
			logger.info("Will override the maxConnectionPoolSize to " + propValue + " from the system property 'mule.sftp.transport.maxConnectionPoolSize'.");
	...
	public MessageReceiver createReceiver(Service component, InboundEndpoint endpoint) throws Exception
	{
		...
		return serviceDescriptor.createMessageReceiver(this, component, endpoint, new Object[]{polling});
	}
	...
	protected synchronized ObjectPool getClientPool(ImmutableEndpoint endpoint)
	{
		...
		} else
		{
			if (logger.isDebugEnabled())
			{
				logger.debug("Using existing pool for endpoint " + endpoint.getEndpointURI() +
  ...
  // Need this method to be public for SftpNotifier
	...
	/**
	 * @return the max connection pool size. If the system parameter mule.sftp.transport.maxConnectionPoolSize is set, that value will be used instead.
	 */
