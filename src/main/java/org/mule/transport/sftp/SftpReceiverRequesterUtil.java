...
/**
 * Contains reusable methods not directly related to usage of the
 * jsch sftp library (they can be found in the class SftpClient).
 *
...
public class SftpReceiverRequesterUtil
{
	...
	public String[] getAvailableFiles(boolean onlyGetTheFirstOne) throws Exception
	{
		...
		try
		{
			...
			{
				// Skip if no match.
				// Note, Mule also uses this filter. We use the filter here because we don't want to
				// run the below tests (checkFileAge, sizeCheckDelayMs etc) for files that Mule
				// later should have ignored. Thus this is an "early" filter so that improves performance.
		...
		if (!"".equals(archive))
		{
			...
			} else
			{
				return archiveFileUsingTempDirs(
					archive, archiveTmpReceivingDir, archiveTmpSendingDir, is, fileNamePart, archiveFile);
			}
		}
	 ...
	 * @param sizeCheckDelayMs Wait time (in ms) between size-checks to determine if a file is ready to be processed.
